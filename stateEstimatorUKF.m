...
if(isempty(xBark))
  ...
  PBark = diag([2*diag(P.sensorParams.RpL); 
                0.001*ones(3,1); ...
                2*P.sensorParams.sigmaC^2*ones(3,1); ...
...
for ii=1:Nf
  if(~isnan(M.rxMat(ii,1)))
    ...
    % viCtildeu is the measured unit vector pointing from the camera frame
    % origin to the 3D feature point, expressed in the camera (C) frame
    zk = [zk; viCtildeu];
